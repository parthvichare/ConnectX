  ...
  useEffect(() => {
    if (!socket || !messageTo) return; // Ensure socket is available
      ...
      if (socket?.socket) {
        ...
        socketConnect.emit("FetchMessage", { initialConvoId });

        // socketConnect.on("AllMessages", (data: { messages: MessageFormat[] }) => {
        //   setMessage(data.messages);
        // });
  
        socketConnect.on("NewMessage", ({ currentMessage }) => {
          if (currentMessage) {
            const { conversationId } = currentMessage;
            // Navigate to the private chat for the conversation
            navigate(`/ConnectX/private-chat/${conversationId}`,{state:{messagesendTo: userDetail}});
          }
        });
      }
